...
impl Glues {
    ...
    pub async fn add_directory(&mut self, parent_id: DirectoryId, name: String) {
        table("Directory")
            .insert()
            .columns(vec!["parent_id", "name"])
            .values(vec![vec![uuid(parent_id), text(name)]])
            .execute(&mut self.glue)
            .await
            .unwrap();
    }
    ...
    pub async fn move_directory(&mut self, directory_id: DirectoryId, parent_id: DirectoryId) {
        table("Directory")
            .update()
            .filter(col("directory_id").eq(directory_id))
